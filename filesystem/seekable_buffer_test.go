...
func TestSeekableBuffer_Write_FromEmpty(t *testing.T) {
    ...
    if n != len_ {
        t.Fatalf("Exactly (%d) bytes weren't written: (%d)", len_, n)
    }
    ...
    if position != 0 {
        t.Fatalf("The seek did not return a position of (%d): (%d)", len_, position)
    }
    ...
    if n != len_ {
        t.Fatalf("Exactly (%d) bytes were not read: (%d)", len_, n)
    }
...
func TestSeekableBuffer_Write_Append(t *testing.T) {
    ...
    if n != len_ {
        t.Fatalf("Exactly (%d) bytes weren't written (1): (%d)", len_, n)
    }
    ...
    if n != len_ {
        t.Fatalf("Exactly (%d) bytes weren't written (2): (%d)", len_, n)
    }
    ...
    buffer = buffer[:len_*2]

    if bytes.Compare(buffer[:len_], data) != 0 {
...
func TestSeekableBuffer_Write_Append_SeekAwayAndBack(t *testing.T) {
    ...
    position, err = sb.Seek(int64(len_), os.SEEK_SET)
    log.PanicIf(err)

    if position != int64(len_) {
        t.Fatalf("Far seek position not correct: (%d) != (%d)", position, len_)
    }
...
func TestSeekableBuffer_Seek_End_Nonempty(t *testing.T) {
    ...
    if position != int64(len_) {
        t.Fatalf("The seek did not move to the last byte of a non-empty byte: (%d) != (%d)", position, len_)
    }
}
...
func TestSeekableBuffer_Seek_Set_Nonempty(t *testing.T) {
    ...
    if position != 0 {
        t.Fatalf("The seek did not move to the first byte of a non-empty file: (%d) != (%d)", position, len_)
    }
}
...
func TestSeekableBuffer_Seek_Cur_Nonempty(t *testing.T) {
    ...
    if position != 5 {
        t.Fatalf("The seek did not stay on the last byte of a non-empty file: (%d) != (%d)", position, len_)
    }
}
